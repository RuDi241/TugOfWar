...
int main(int argc, char *argv[])
{
    if (argc != 10)
    {
        fprintf(stderr, "Usage: %s <to_referee_write_fd> <to_player_read_fd> <energy_min> <energy_max> <energy_loss_min> <energy_loss_max> <fall_prob> <min_recovery_time> <max_recovery_time>\n", argv[0]);
        exit(EXIT_FAILURE);
    }

    // printf("from player %d %s %s %s %s %s %s %s %s %s\n", getpid(), argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], argv[9]);
    ...
    // printf("min_energy_loss: %d, max_energy_loss: %d\n", min_energy_loss, max_energy_loss);
    // printf("min_recovery_time: %d, max_recovery_time: %d\n", min_recovery_time, max_recovery_time);
    // printf("probability_fall: %d\n", probability_fall);
    // printf("min_energy: %d, max_energy: %d\n", min_energy, max_energy);
    // printf("\n");

    // Send initial energy to the referee
    write(player.to_referee_fd[1], &player.energy, sizeof(player.energy));             // Write first integer
    write(player.to_referee_fd[1], &player.fall_timeout, sizeof(player.fall_timeout)); // Write second intege
...
void SIGUSR1_handler(int sig_num)
{
    ...
    if (read(player.to_player_fd[0], &lastRoundOutcome, sizeof(lastRoundOutcome)) == -1)
    {                                                         // read if player is winner
        perror("Error reading last Round Outcome from pipe"); // Print the error message based on errno
        return;
    }
    ...
    if (write(player.to_referee_fd[1], &player.energy, sizeof(player.energy)) == -1)
    {
        perror("Error writing energy to referee");
        return;
    }

    if (write(player.to_referee_fd[1], &player.fall_timeout, sizeof(player.fall_timeout)) == -1)
    {
...
void SIGUSR2_handler(int sig_num)
{
    ...
    {
        player.fall_timeout--;
        if (write(player.to_referee_fd[1], &energy_if_fall, sizeof(energy_if_fall)) == -1)
        {
