...
int create_team_processes(Team *team, TeamConfig *team_config) {
  ...
  for (int i = 0; i < team_config->num_players; i++) {
    ...
    if (team->players[i].pid < 0) {
      ...
      int fall_percentage =
          (int)(team_config->fall_probability *
                100); // convert fall_probability to percentage

      sprintf(initial_energy_first, "%d",
              team_config->initial_energy_range.first);
      sprintf(initial_energy_second, "%d",
              team_config->initial_energy_range.second);
      sprintf(energy_decay_first, "%d", team_config->energy_decay_range.first);
      sprintf(energy_decay_second, "%d",
      ...
      execl("./bin/player", "player", write_fd, read_fd, initial_energy_first,
            initial_energy_second, energy_decay_first, energy_decay_second,
            fall_probability_str, fall_time_first, fall_time_second, NULL);
      perror("execl failed");
      return EXEC_ERROR;
    }
  }
...
int receive_data_from_team(Team *team) {
  for (int i = 0; i < team->size; i++) {
    if (read(team->players[i].to_referee_fd[0], &team->players[i].energy,
             sizeof(team->players[i].energy)) <= 0) {
      perror("Failed to receive data from Player.");
      close(team->players[i].to_referee_fd[0]); // Clean up
      return READ_PIPE_ERROR;
    }
    if (read(team->players[i].to_referee_fd[0], &team->players[i].fall_timeout,
             sizeof(team->players[i].fall_timeout)) <= 0) {
      perror("Failed to receive data from Player.");
      close(team->players[i].to_referee_fd[0]); // Clean up
      return READ_PIPE_ERROR;
    }
    printf("Player %d: Energy: %d, Fall Timeout: %d\n", team->players[i].pid,
           team->players[i].energy, team->players[i].fall_timeout);
  }
...
int announce_result_to_team(Team *team, int lastRoundOutcome) {
  for (int i = 0; i < team->size; i++) {
    ssize_t bytes_written = write(team->players[i].to_player_fd[1],
                                  &lastRoundOutcome, sizeof(lastRoundOutcome));
    if (bytes_written == -1) {
